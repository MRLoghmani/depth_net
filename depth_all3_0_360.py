...
for n in range (1,n_samples+1):
    size=random.random()*0.75 + 0.9   #1.6 as maximum 
    CamDist=ZoomDist*CamStartDist/(size)
    counter=counter+1   
    #if axis is not None:
    #    ob.dimensions[axis] = old_val
    #axis = random.randint(0,2)
    #old_val = ob.dimensions[axis]
    #ob.dimensions[axis] = old_val * (1 + random.random()*0.2 - 0.1)
