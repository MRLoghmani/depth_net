...
python feature_extractor.py /home/hades/CIN-DB_normalized CIN_DB/all_depth.txt $DEPLOY $CAFFE_MODEL $NORM_NAME --gpu_id 0 --batch-size $BSIZE --center_data --layer_name $LAYER

echo Extracting CIN-DB original to $ORIG_NAME
python feature_extractor.py /home/hades/CIN-DB CIN_DB/all_depth.txt $DEPLOY $CAFFE_MODEL $ORIG_NAME --gpu_id 0 --batch-size $BSIZE --center_data --layer_name $LAYER
