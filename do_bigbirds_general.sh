...
NORM_NAME=${FEAT_FOLDER}bigbirds_${JOB_ID}_${layer}.pkl

echo Extracting Washington normalized to $NORM_NAME
python -u feature_extractor.py ${4} bigbird/all_depth.txt $DEPLOY $CAFFE_MODEL $NORM_NAME --center_data --gpu_id 0 --batch-size $BSIZE --layer_name ${layer}


echo "Running SVM on $NORM_NAME"
SECONDS=0
python -u svm_baseline_parallel.py bigbird/ $NORM_NAME --splits $N_SPLITS --jobs 4 # --kernel_name washington_normalized_kernel_${JOB_ID}
