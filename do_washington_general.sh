...
NORM_NAME=${FEAT_FOLDER}/${JOB_PREFIX}washington_${JOB_ID}_${layer//\//_}.pkl

echo Extracting Washington normalized to $NORM_NAME
python -u feature_extractor.py ${4} Washington/all_depth_clean.txt $DEPLOY $CAFFE_MODEL $NORM_NAME --center_data --gpu_id 1 --batch-size $BSIZE --layer_name ${layer}
