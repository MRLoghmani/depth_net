...
python feature_extractor.py ../CIN/CIN-DB_normalized CIN_DB/all_depth.txt $DEPLOY $CAFFE_MODEL $NORM_NAME --batch-size 256 --center_data --layer_name $LAYER

echo Extracting CIN-DB original to $ORIG_NAME
python feature_extractor.py ../CIN/CIN-DB CIN_DB/all_depth.txt $DEPLOY $CAFFE_MODEL $ORIG_NAME --gpu_id 0 --batch-size 256 --center_data --layer_name $LAYER
source deactivate
echo "Running SVM on $NORM_NAME"
SECONDS=0
python svm_baseline_parallel.py CIN_DB/ $NORM_NAME --splits 10 --jobs 3 --split_prefix cin_db_depth_ --normalize
echo "Took $SECONDS seconds"

#SECONDS=0
#echo "Running SVM on $ORIG_NAME"
python svm_baseline_parallel.py CIN_DB/ $ORIG_NAME --splits 10 --jobs 3 --split_prefix cin_db_depth_ --normalize
#echo "Took $SECONDS seconds"
