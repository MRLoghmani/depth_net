...
python feature_extractor.py ../Washington/rgbd-normalized_gray/ Washington/all_depth_clean.txt $DEPLOY $CAFFE_MODEL $NORM_NAME --center_data --batch-size $BSIZE
...
SECONDS=0
python svm_baseline_parallel.py Washington/splits/ $NORM_NAME --splits $N_SPLITS --jobs $N_JOBS
echo "Took $SECONDS seconds"

SECONDS=0
echo "Running SVM on $ORIG_NAME"
python svm_baseline_parallel.py Washington/splits/ $ORIG_NAME --splits $N_SPLITS --jobs $N_JOBS
echo "Took $SECONDS seconds"
